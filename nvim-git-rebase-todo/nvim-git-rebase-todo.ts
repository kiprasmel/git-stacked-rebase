...
export default function nvimGitRebaseTodo(plugin: NvimPlugin): void {
	...
	const hideWindowAsync = async (): Promise<void> => {
		/**
		 * overkill
		 */
		// if (!gOurWindow) {
	...
	type SetWindowRelativeToCursorAsyncOpts = WH;
	const getRelativeWindowOptionsAsync = async ({
		width, //
		height,
	}: SetWindowRelativeToCursorAsyncOpts): Promise<OpenWindowOptions> => {
		/**
	...
	const initWindowAsync = async ({ buffer, width, height }: InitWindowOpts): Promise<void> => {
		...
		const opts: OpenWindowOptions = config.relativeToCursor
			? {
					...(await getRelativeWindowOptionsAsync({ width, height })),
			  }
			: ({
					relative: "win",
					win: relWin.id,
	...
	const updateWindowAsync = async ({ buffer, width, height }: InitWindowOpts): Promise<void> => {
		...
		if (width === 0 && height === 0) {
			...
			if (config.relativeToCursor) {
				const opts = await getRelativeWindowOptionsAsync({
					width, //
					height,
				});
